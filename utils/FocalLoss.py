...
class FocalLoss(nn.Module):
    ...
    def forward(self, inputs, targets):  # p_conf为判定此时传入的参数是否为confidence层的，如果是需要进行进一步的操作
        N = inputs.size(0)
        # print(N)
        C = inputs.size(1)
        P = F.softmax(inputs)
...
class FocalLoss_confidence(nn.Module):  # confidence版本的FocalLoss,对confidence的定制版本，对target和inputs的处理会有所不同
    r"""
        This criterion is a implemenation of Focal Loss, which is proposed in
        Focal Loss for Dense Object Detection.
    ...
    def __init__(self, class_num, alpha=None, gamma=2, size_average=True):
        super(FocalLoss_confidence, self).__init__()
        if alpha is None:
            self.alpha = Variable(torch.ones(class_num, 1))
        else:
            if isinstance(alpha, Variable):
    ...
    def forward(self, inputs, targets):  # p_conf为判定此时传入的参数是否为confidence层的，如果是需要进行进一步的操作
        ...
        inputs = torch.cat((inputs, one_minus_inputs), dim=1)

        targets = targets.view(-1, 1)
        one_minus_targets = 1 - targets
        targets = torch.cat((targets, one_minus_targets), dim=1)
